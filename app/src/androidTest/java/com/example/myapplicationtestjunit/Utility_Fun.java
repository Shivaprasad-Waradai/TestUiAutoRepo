...
public class Utility_Fun   {
    ...
    static Device_Veriables dv;
    public Utility_Fun(UiDevice uiDevice) {
    ...
    public static void ufMenu_srch_App(String AppName) throws Exception {
        try {
            ...
            UiObject device_main_menu_Key = new UiObject(new UiSelector()
                    .resourceId(dv.DEVICE_MAIN_MENU_resourceId));//Frontier Veriable Values are present in Device_Veriable Class

            UiObject search_App = new UiObject(new UiSelector()
                    .resourceId(dv.DEVICE_SEARCH_APP_FROM_MAIN_MENU_resourceId));//Frontier
    ...
    public void add_numbers(String ContactName, String SAVING_NUMBER) {
        try {
            ...
            // .resourceId(CREATE_CONTACT_resourceId));
            //  UiObject create_contact = new UiObject(new UiSelector()
            // .className("android.widget.ImageButton").descriptionContains("Create contact"));

            // UiObject create_contacts = new UiObject(new UiSelector()
            //  .resourceId(CREATE_CONTACTS_resourceId));

            //   create_contacts.click();
            //  Thread.sleep(5000);
            //  UiObject create_contac = new UiObject(new UiSelector()
            //  .descriptionContains("Create contact"));
            ...
            UiObject entering_first_name = new UiObject(new UiSelector()
                    .text("First name"));   //Entering First name
            entering_first_name.setText(ContactName);
            Thread.sleep(2000);
            //getInstrumentation().sendKeyDownUpSync(KeyEvent.KEYCODE_BACK);//*[@resource-id='']
            // mUiDevice.swipe(300, 900, 300, 300, 5);
            // Thread.sleep(5000);
            ...
            UiObject phone_number = new UiObject(new UiSelector()
                    .className(dv.DATA_ENTERING_classId).text("Phone"));//Entering Phone Number
            phone_number.click();
    ...
    public void deleting_numbers(String ContactName)  throws Exception {
        try {
            ...
            UiObject search_option = new UiObject(new UiSelector()
                    .resourceId(dv.SEARCH_OPTION_IN_CONTACTS_resourceId));
            UiObject search_number = new UiObject(new UiSelector()
                    .resourceId(dv.SEARCH_NUMBER_resourceId));
            UiObject select_number = new UiObject(new UiSelector()
                    .resourceId(dv.SELECT_NUMBER_resourceId));
            ...
            if (NoRecentApps.exists()) {
                ...
                while (flag) {
                    ...
                    if (ClearAll.exists()) {
                        ...
                        mUiDevice.swipe(400, 200, 400, 1170, 10);
                }
    ...
    public void Sim_Selection()   {
        try {
            ...
            UiObject Selecting_Sim_Cards = new UiObject(new UiSelector()
                    .className(dv.NETWORK_AND_INTERNET_OPTION_className).text("SIM cards")); //Inside Setting selecting SIM cards
            UiObject Selecting_Mobile_data = new UiObject(new UiSelector()
                    .className(dv.NETWORK_AND_INTERNET_OPTION_className).text("Mobile data")); //Inside Setting Mobile data SIM card
            UiObject Selected_sim1 = new UiObject(new UiSelector()
                    .className(dv.SETTINGS_MENU_className).index(0)); //Inside Setting selecting SIM cards
    ...
    public void Sim2_Selection()   {
        try {
            ...
            UiObject Selecting_Mobile_data = new UiObject(new UiSelector()
                    .className(dv.NETWORK_AND_INTERNET_OPTION_className).text("Mobile data")); //Inside Setting Mobile data SIM card
            UiObject Selected_sim1 = new UiObject(new UiSelector()
                    .className(dv.SETTINGS_MENU_className).index(1)); //Inside Setting selecting SIM cards
    ...
    public void Alarams_Added()   {
        try {
            UiObject Selecting_Alaram_Option = new UiObject(new UiSelector() //Selecting Alaram Button
                    .className(dv.SELECTING_ALARAM_OPTION_ClassName).descriptionContains("Alarm"));
